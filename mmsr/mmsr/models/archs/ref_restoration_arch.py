...
class WindowAttention_SA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe, locality_strength,          use_local_init, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., fc=None):
        ...
        self.mu = nn.Parameter((torch.empty( 2, self.num_heads, self.head_dim).normal_(mean = 0.0, std = 1.))*torch.tensor([0.,1.])[:, None, None], requires_grad= True)
    ...
    def forward(self, q, k1, k2, v, mask):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        ...
        dist_min1 = (-1/(2*self.scale))*dist_min 
        ...
        dist_min = torch.minimum(QK1_distance, QK2_distance)
        dist_min2 = (-1/(2*self.scale))*dist_min 

        # QK_distance = (-1/(2*self.scale))*self.dist._sq_dist(q, k1 - self.mu[0][None, :, None, :], postprocess = False) 
        # QK2_distance = (-1/(2*self.scale))*self.dist._sq_dist(q, k2 - self.mu[1][None, :, None, :], postprocess = False) 
        gating = self.gating_param.view(1,-1,1,1)
        attn = torch.clamp((1.-torch.sigmoid(gating)) * dist_min1, min = 0., max = 1.) + torch.clamp(torch.sigmoid(gating) * dist_min2, min = 0., max = 1.)
        # attn = torch.exp(dist_min1)*torch.clamp(self.pi, min = 0., max = 1.)[0] + torch.exp(dist_min2)*torch.clamp(self.pi, min = 0., max = 1.)[1]
...
class RefSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, locality_strength, use_local_init, no_off, fixed_pe, window_size, shift_size=4, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=GeLU(), norm_layer=ScaleNorm, fused_window_process=False, fc=None):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, ref):
        ...
        x1, ref1, _ = self.attn_SA(q1_windows, k1_windows, k2_windows, v2_windows, self.attn_mask)
        x2, ref2, _ = self.attn_SA(q1_windows, k1_windows, k2_windows, v2_windows, self.attn_mask)
        ...
        x3, ref3, _ = self.attn_SA(q3_windows, k3_windows, k4_windows, v4_windows, self.attn_mask)
        x4, ref4, _ = self.attn_SA(q3_windows, k3_windows, k4_windows, v4_windows, self.attn_mask)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe, locality_strength, use_local_init,               mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=ScaleNorm, downsample=None, upsample=None, use_checkpoint=False,
                 fused_window_process=False, fc=None):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=40, patch_size=4, in_chans=6, in_channel=3, num_classes=65536, embed_dim=48, depths=[2, 2, 2, 2], num_heads=[6, 24, 24, 24],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
                 norm_layer=ScaleNorm, ape=False, upsample=False, patch_norm=True,
    ...
    def forward_features(self, lr, ref1, ref, ref_g, img_ref_feat, truncation=1, truncation_latent=None): 
        ...
        # head_lr = self.conv_ref1(torch.cat((lr, ref1), 1))
        # for i in range(4): 
        #     head_lr = self.stages[i](head_lr)
        ...
        # B, C, H, W = lr.shape
        # x_lr = lr.permute(0, 2, 3, 1).contiguous().reshape(B, H * W, C)
