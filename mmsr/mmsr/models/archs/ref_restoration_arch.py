...
class RefSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size, shift_size=4, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=GeLU(), norm_layer=nn.LayerNorm, fused_window_process=False, fc=None):
        super().__init__()
        self.dim = dim
        ...
        self.attn_SA = WindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,fc=fc)
    ...
    def forward(self, x, ref):
        ...
        x1, ref1, _ = self.attn(q1_windows, k1_windows, k2_windows, v1_windows, self.attn_mask)
        x2, ref2, _ = self.attn(q1_windows, k1_windows, k2_windows, v1_windows, self.attn_mask)
        ...
        x3, ref3, _ = self.attn(q3_windows, k3_windows, k4_windows, v3_windows, self.attn_mask)
        x4, ref4, _ = self.attn(q3_windows, k3_windows, k4_windows, v3_windows, self.attn_mask)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=40, embed_dim=48, depths=[2, 2, 2, 2], num_heads=[12, 24, 24, 24],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
        ...
        self.head = nn.Conv2d(3, in_channels[-1], kernel_size=3, stride=1, padding=1)
    ...
    def forward_features(self, lr, ref_s, ref_m, ref_g): 
        shortcut = lr
      
        lr = self.content_extractor(lr)
       
        ref_s = self.head(ref_s)
        ref_m = self.head(ref_m)
        ref_g = self.head(ref_g)
                     
        base = F.interpolate(shortcut, None, 4, 'bilinear', False)

        B, C, H, W = lr.shape
        lr = lr.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)
        ref_s = ref_s.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)

        lr = self.to_patch_embedding(lr)
        ref_s = self.to_patch_embedding(ref_s)
        ...
        ref_m = ref_m.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)
        ref_m = self.to_patch_embedding(ref_m)
    ...
    def forward(self, lr, ref_s, ref_m, ref_g):
        out = self.forward_features(lr, ref_s, ref_m, ref_g)
