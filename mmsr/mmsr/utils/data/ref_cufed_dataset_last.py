...
class RefCUFEDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            # img_in, img_ref = augment([img_in, img_ref], self.opt['use_flip'],
            #                           self.opt['use_rot'])
            # # img_ref = augment([img_ref], self.opt['use_flip'],
            # #                           self.opt['use_rot'], self.opt['use_jitter'], 
            # #                           self.opt['use_blur'], self.opt['use_gray'])
            ...
            # if img_in_h != target_h or img_in_w != target_w:
            #     padding = True
            ...
            # # img_in_t = center_crop(img_in_pil, target_size=(80,80))
            # # img_ref_t = center_crop(img_ref_pil, target_size=(160,160))
            ...
            # img_ref_pil = Image.fromarray(
            #     cv2.cvtColor(img_ref_pil.astype(np.uint8), cv2.COLOR_BGR2RGB))
            # # Comment the couple of lines below for test
            # img_in_pil = center_crop(img_in_pil, target_S=(160,160))
            # img_ref_pil = center_crop(img_ref_pil, target_S=(160,160))

            # # img_in_t = random_crop(img_in_pil, random_size=(80,80))
            # # img_ref_t = random_crop(img_ref_pil, random_size=(160,160))
        ...
        img_in_pad = img_in_pil
        img_ref_pad = img_ref_pil

        img_in_pad = cv2.cvtColor(np.array(img_in_pad), cv2.COLOR_RGB2BGR)
        img_in_pad = img_in_pad.astype(np.float32) / 255.

        # img_ref_t = cv2.cvtColor(np.array(img_in_t), cv2.COLOR_RGB2BGR)
        # img_ref_t = img_ref_t.astype(np.float32) / 255.
        
        img_ref_pad = cv2.cvtColor(np.array(img_ref_pad), cv2.COLOR_RGB2BGR)
        img_ref_pad = img_ref_pad.astype(np.float32) / 255.
        ...
        # img_ref_t = cv2.cvtColor(np.array(img_ref_t), cv2.COLOR_RGB2BGR)
        # img_ref_t = img_ref_t.astype(np.float32) / 255.
        ...
        img_ref_g = center_crop(img_ref_pil, target_S=(160,160))

        # img_ref_lq = img_ref_pil.resize((lq_w, lq_h), Image.BICUBIC)

        # img_in_lq_h, img_in_lq_w = img_in_lq.size
        # img_ref_lq_h, img_ref_lq_w = img_ref_lq.size
        # padding = False
        # if img_in_lq_h != target_size[0]//4 or img_in_lq_w != target_size[1]//4:
        #     img_in_lq = img_padding(img_in_lq, 8, 8, 8, 8)
        #     img_ref_lq = img_padding(img_ref_lq, 8, 8, 8, 8)
        img_in_up = img_in_lq.resize((gt_w, gt_h), Image.BICUBIC)
        img_ref_up = img_ref_lq.resize((gt_w, gt_h), Image.BICUBIC)
        ...
        img_ref_lq1, img_ref_g, img_in_up, img_in_pad, img_ref_pad, img_gt_val, img_ref_val, img_in, img_in_lq, img_ref, img_ref_lq = totensor(  # noqa: E501
            [img_ref_lq1, img_ref_g, img_in_up, img_in_pad, img_ref_pad, img_gt_val, img_ref_val, img_in, img_in_lq, img_ref, img_ref_lq],
            bgr2rgb=True,
            float32=True)
        ...
        return_dict = {
            ...
            'img_in_pad': img_in_pad,
            'img_ref_pad': img_ref_pad,
