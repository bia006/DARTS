...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.conv = nn.Conv2d(48, 96, 1, 1, 0) 
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # ## If not deformable attn block, then q is replaced with x from the previous command statement
...
class RefSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        x3, _, _ = self.attn(q3_windows, k4_windows, v4_windows, self.attn_mask3)
        x4, _, _ = self.attn(q4_windows, k3_windows, v3_windows, self.attn_mask4)
        ...
        x = torch.cat([x1.reshape(B, H * W, C//2), x3.reshape(B, H * W, C//2)], dim=2)
        # x_ref = torch.cat([x3.reshape(B, H * W, C//2), x4.reshape(B, H * W, C//2)], dim=2)
        
        # x = self.linear(torch.cat((x_x, x_ref), 2)) 
        x = self.proj(x)      
    ...
    def get_window_qkv(self, qkv):
        ...
        C = q.shape[-1]
        gate_q = 1 + torch.sigmoid(q)
        gate_k = 1 + torch.sigmoid(k)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe,                  mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, upsample=None, use_checkpoint=False,
        ...
        if upsample is not None:
            self.upsample = upsample(input_resolution, dim=dim, out_dim=dim-48)
        else:
            self.upsample = None
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=40, patch_size=4, in_chans=6, in_channel=3, num_classes=65536, embed_dim=48, depths=[2, 2, 2, 2], num_heads=[24, 24, 24, 24],
        ...
        self.conv4 = nn.Conv2d(in_channels[-1]*4, 8 * in_channels[-1], 1, 1, 0)
        self.upsample = nn.PixelShuffle(2)
        ...
        self.conv_ref1 = nn.Conv2d(in_channels[-1]*2+192, in_channels[-1]+96, 3, 1, 1)
        self.conv_ref2 = nn.Conv2d(in_channels[-1]*2+96, in_channels[-1]+48, 3, 1, 1)
        self.conv_up = nn.Conv2d(in_channels[-1], in_channels[-1]*4, 3, 1, 1)
        self.conv_31 = nn.Conv2d(256, in_channels[-1]+96, 3, 1, 1)
        self.conv_21 = nn.Conv2d(128, in_channels[-1]+48, 3, 1, 1)
        ...
        self.content_extractor = ContentExtractor(
            in_nc=3, out_nc=3, nf=in_channels[-1]+96, n_blocks=16)
    ...
    def forward_features(self, lr, ref1, ref, ref_g, img_ref_feat, truncation=1, truncation_latent=None): 
        ...
        head_lr = self.conv_ref1(torch.cat((head_lr, ref1), 1))
        # for i in range(4): 
        ...
        #     lr, ref1 = layer(lr, ref1)         
        lr_1 = lr.transpose(-1, -2).reshape(lr.shape[0], self.embed_dim+48, shortcut.shape[2]*2, shortcut.shape[3]*2).contiguous() #240
        # lr_down = self.conv_down(lr_1)
        lr_1 = self.conv_ref2(torch.cat((lr_1, ref), 1))

        # for i in range(4): 
        #     lr_down = self.stages[i](lr_down)
...
class SwinTransformer_T(nn.Module):
    ...
    def __init__(self, img_size=40, patch_size=4, in_chans=6, in_channel=3, num_classes=65536, embed_dim=48, depths=[2, 2, 2, 2], num_heads=[24, 16, 12, 6],
        ...
        self.norm = nn.LayerNorm(in_channels[-1]) # final norm layer
        self.mlp = Mlp(in_channels[-1], in_channels[-1], act_layer=nn.GELU, drop=0.)
        self.act = nn.GELU()
        self.proj = nn.Conv2d(in_channels[-1], in_channels[-1], kernel_size=4, stride=4)
        self.proj_1 = nn.Conv2d(in_channels[-1], in_channels[-1], kernel_size=1, stride=1)
        self.proj_2 = nn.Conv2d(in_channels[-1], in_channels[-1], kernel_size=2, stride=2)
    ...
    def forward_features(self, lr, ref1, ref, ref_g, img_ref_feat, truncation=1, truncation_latent=None): 
        ...
        lr = lr.transpose(-1, -2).reshape(lr.shape[0], self.embed_dim//2, shortcut.shape[2]*4, shortcut.shape[3]*4).contiguous() 
