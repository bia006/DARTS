...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=[56, 224], patch_size=[4, 1], in_chans=48, in_channel=3, num_classes=65536, embed_dim=[48, 48], depths=[3, 3, 9, 3, 3], num_heads=[3, 6, 12, 24, 12], layer_scale_init_value=1e-6, head_init_scale=1.,
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, upsample=False, patch_norm=True,
                 use_checkpoint=False, use_deformable_block: bool = True, fused_window_process=False, stage_idx=[0, 1, 2, 3, 4], strides=[1,1, 1, 1, 4], groups=[1, 1, 3, 6, 6], use_pes=[True, True, True, True, True], 
        ...
        stem = nn.Sequential(
            nn.Conv2d(in_channel, dims[0], kernel_size=4, stride=2, padding=1),
        ...
        for i in range(4):
            upsample_layer = nn.Sequential(
                    ...
                    nn.PixelShuffle(2),
                    nn.Conv2d(dims[i]//4, dims[i], kernel_size=3, stride=1, padding=1),
                    # nn.Upsample(scale_factor=2, mode='bilinear'),
                    # nn.ConvTranspose2d(dims[i], dims[i+1], kernel_size=2, stride=2),
            )
            self.downsample_layers.append(upsample_layer)
        ...
        self.layer_ref = BasicLayer(dim=dims[3],
        ...
        self.conv_head = nn.Conv2d(3, dims[0], 3, 1, 1)
        
        self.out = nn.Conv2d(48, 3, 1, 1, 0)
    ...
    def forward_features(self, lr, ref, ref_g): 
        student_output = []    
        shortcut = lr
        ref_g = self.conv_head(ref_g)

        for i in range(4):
            lr = self.downsample_layers[i](lr)
            lr = self.stages[i](lr)
            x = self.norm(lr.mean([-2, -1]))  # B L C
        ...
        lr = self.patch_embed_ref(lr)
        if self.ape:
            lr = lr + self.absolute_pos_embed_ref
            ref_g = ref_g + self.absolute_pos_embed_ref
        lr = self.pos_drop(lr)
        ref_g = self.pos_drop(ref_g)
        
        lr, ref_g = self.layer_ref(lr, ref_g)
        lr = lr.transpose(-1, -2).contiguous().view(lr.shape[0], 48, 224, 224)
        lr = self.out(lr)
