...
def normalize_grid(grid, dim = 1, out_dim = -1):
    ...
    h, w = grid.shape[-2:]
    _, grid_h, grid_w, _ = grid.unbind(dim = dim)
...
def unfold(input: torch.Tensor,
    ...
    # Unfold input
    output: torch.Tensor = input.unfold(dimension=0, size=8, step=8) \
        .unfold(dimension=0, size=3, step=3).unfold(dimension=1, size=7, step=7).permute(4, 0, 1, 3, 5, 2)
    

    # Reshape to [batch size * windows, channels, window size, window size]
    B, H, W, H_, W_, C = output.shape
    output = output.reshape(B, C, H*W, H_*W_)
...
class DeformableAttention2D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.unfold = unfold(input, window_size=8, kernel_size=3)
    ...
    def forward(self, x, return_vgrid = False):
        ...
        g - offset groups
        """
        x = self.unfold = unfold(x, window_size=8, kernel_size=3)
        print ('x_da',x.shape)
