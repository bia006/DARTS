...
#         _, self.feature_extraction_image1 = ContrasExtractorLayer()
#         # _, self.feature_extraction_image2 = ContrasExtractorLayer()
...
#         self.fc = nn.Linear(num_classes, 47)
...
#             x = self.stages[i](x)
#             feature_extraction.append(x)
#         return self.norm(x.mean([-2, -1])), feature_extraction # global average pooling, (N, C, H, W) -> (N, C)

#     def forward(self, x):
#         x, feature_extraction = self.forward_features(x)
#         x = self.fc(self.head(x))
#         return x, feature_extraction
