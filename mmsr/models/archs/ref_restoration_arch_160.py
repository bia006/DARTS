...
class RefSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        x1, _, _ = self.attn(q1_windows, k1_windows, v2_windows, self.attn_mask1)
        # x2, _, _ = self.attn(q1_windows, k2_windows, v2_windows, self.attn_mask2)
        ...
        # ref = shortcut + ref + self.drop_path(ref)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, n_groups, stage_idx, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe,                  mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, upsample=None, use_checkpoint=False,
        ...
        if upsample is not None:
            self.upsample = upsample(input_resolution, dim=dim, out_dim=dim//2)
        else:
            self.upsample = None
    ...
    def forward(self, x, ref):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, ref)
            else:
                x = blk(x, ref)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=40, patch_size=1, in_chans=3, embed_dim=768, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        ...
        self.out = nn.Conv2d(192, 3, 1, 1, 0)
    ...
    def forward_features(self, lr, ref): 
        student_output = []      
        lr = self.patch_embed(lr)
        # print ('S111', lr.shape)
        ref = self.patch_embed(ref)
        # print ('ref11', ref.shape)
        if self.ape:
        ...
        for layer in self.layers:
            ...
            if new_lr.shape[2] == self.embed_dim // 4:
                new_lr = self.upsample_4(new_lr)
            elif new_lr.shape[2] == self.embed_dim // 2:
                new_lr = self.upsample_2(new_lr)
            # print ('new_lr', new_lr.shape)

            student_output.append(new_lr)
        lr = self.norm(lr)  # B L C

        lr = lr.transpose(-1, -2).contiguous().view(lr.shape[0], self.embed_dim//4, 160, 160)
        lr = self.out(lr)

        return lr, student_output

    def forward(self, lr, ref):
        lr, student_output = self.forward_features(lr, ref)
...
class SwinTransformer_T(nn.Module):
    ...
    def __init__(self, img_size=[40, 80, 160, 160], patch_size=1, in_chans=3, embed_dim=[768, 192, 768, 96], depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=10, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, upsample=False, patch_norm=True,
                 use_checkpoint=False, use_deformable_block: bool = True, fused_window_process=False, stage_idx=[0, 1, 2, 3], strides=[1,1, 1, 1], groups=[1, 1, 3, 6], use_pes=[True, True, True, True], 
    ...
    def forward_features_teacher(self, s_in, t_in):     
        ...
        lr = self.patch_embed_1x(s_in[2])
        ref = self.patch_embed_1x(t_in[2])
        if self.ape:
            lr = lr + self.absolute_pos_embed_1x
            ref = ref + self.absolute_pos_embed_1x
        lr = self.pos_drop(lr)
        ref = self.pos_drop(ref)
        lr, _ = self.layer_1x(lr, ref)
        for i in range (2):
            teacher_output.append(lr)   
